...
type Person = {
  ...
  thumb: string;
  gallery: GalleryItem[];
  /** ★スマホ縦で見せたい2枚だけを指定（なければ gallery 先頭2枚を自動使用） */
  story?: GalleryItem[];
};
...
export default function SchoolIntro() {
  ...
  const onDragEnd = (_ev: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    if (!open) return;
    const dx = info.offset.x ?? 0;

    // ★ story があれば2枚、なければ gallery 先頭2枚で固定
   ...
   * ========================================================================= */
  return (
    <section id="about-gachibun" className="relative isolate overflow-hidden bg-black text-white pt-28 md:pt-40">
      {/* ▼ 背景 */}
          ...
          style={
            {
              ...
              right: "var(--top-edge)",
              transform:
                "translate(calc(var(--ov) + var(--diag)), calc(-1 * (var(--ov) + var(--diag))))",
        ...
        >
          <div className="flex items-center gap-4 md:gap-6 p-3 md:p-6 md:pr-8">
            ...
            <div className="text-left">
              <div
                className="text-xs sm:text-sm md:text-base text-white/85 leading-tight"
                style={{ textShadow: "0 1px 3px rgba(0,0,0,.6)" }}
              >
                {VICE.title}
              </div>
              <div
                className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-extrabold leading-tight break-keep"
          ...
          style={
            {
              ...
              transform:
                "translate(calc(-1 * (var(--ov) + var(--diag))), calc(var(--ov) + var(--diag)))",
            } as React.CSSProperties
          }
        >
          <div className="flex items-center gap-4 md:gap-6 p-3 md:p-6 md:pr-8">
      ...
      {/* 画像 #2 */}
        {/* ▼ ここに概要文を移動（2枚目画像と同じコンテナに入れる） */}
<div className="relative mb-24 md:mb-32 mx-auto max-w-6xl px-4 sm:px-6">
  <motion.p
    className="mb-4 text-sm md:text-base leading-relaxed text-white/90 text-center whitespace-pre-line  [text-shadow:0_2px_3px_rgba(0,0,0,.7)]"
    initial={{ opacity: 0, y: 6 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true, amount: 0.5 }}
    transition={{ duration: 0.4, ease: "easeOut" }}
  >
     {"ガチ文高等学校とは、年に1回の文化祭に重きをおいた高校です。\n“青春の装置”として機能します。"}
  </motion.p>
        <motion.div
              ...
              {(() => {
                ...
                return (
                  <motion.div
                    ...
                    className="relative w-[min(72vw,320px)] aspect-[9/16] overflow-hidden rounded-xl ring-1 ring-white/20 shadow-2xl bg-black"
                    drag="x"
                    ...
                    exit={{ opacity: 0, y: 10 }}
                    onClick={(e) => e.stopPropagation()} // 画像ボックスのクリックは閉じない
                  >
                    <img
                      src={slides[gidx]?.src}
                      alt=""
      ...
      <style jsx global>{`
        ...
        .swipe-hint-r {
          right: 10px;
          animation: nudgeR 1.1s ease-in-out infinite;
        }
        .swipe-hint-l {
          left: 10px;
          animation: nudgeL 1.1s ease-in-out infinite;
        }
        @keyframes nudgeR {
          0%,
          100% {
